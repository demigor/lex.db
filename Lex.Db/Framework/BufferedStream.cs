...
namespace Lex.Db.Streams
{
  public sealed class BufferedStream : Stream
  {
    ...
    public override int Read(byte[] array, int offset, int count)
    {
      ...
      if (n > count) n = count;
      Array.Copy(_buffer, _readPos, array, offset, n);
      _readPos += n;
    ...
    public override void Write(byte[] array, int offset, int count)
    {
      ...
      if (_writePos > 0)
      {
        ...
        if (numBytes > 0)
        {
          if (numBytes > count)
            numBytes = count;
          Array.Copy(array, offset, _buffer, _writePos, numBytes);
          _writePos += numBytes;
      ...
      Array.Copy(array, offset, _buffer, 0, count);
      _writePos = count;
    }
      ...
      */
      ...
      if (_readLen > 0)
      {
        ...
        else if (oldPos - _readPos < pos && pos < oldPos + _readLen - _readPos)
        {
          ...
          Array.Copy(_buffer, _readPos + diff, _buffer, 0, _readLen - (_readPos + diff));
...
namespace Lex.Db.Streams
{
  ...
  public sealed class BufferedStream : Stream
  {
    ...
    int _writePos;     
    readonly int _bufferSize;   
    readonly Awaiter _awaiter = new Awaiter(true);
    readonly bool _canSeek, _canRead, _canWrite;

    const int DefaultBufferSize = 4096;

    public BufferedStream(IRandomAccessStream stream)
      : this(stream, DefaultBufferSize)
    {
    }

    public BufferedStream(IRandomAccessStream stream, int bufferSize)
    {
      if (stream == null)
        throw new ArgumentNullException("stream");
      if (bufferSize <= 0)
    ...
    public override long Position
    {
      ...
      {
        if (_s == null) ErrorStreamIsClosed();
        if (!_canSeek) ErrorSeekNotSupported();
        //              return _s.Seek(0, SeekOrigin.Current) + (_readPos + _writePos - _readLen);
        return (long)_s.Position + (_readPos - _readLen + _writePos);
      }
      set
      {
        if (value < 0) throw new ArgumentOutOfRangeException("value");
    ...
    public override void Flush()
    {
      ...
      else if (_readPos < _readLen && _canSeek)
      {
        FlushRead();
      }
      _readPos = 0;
    ...
    {
      if (_readPos - _readLen != 0)
        _s.Seek((ulong)(_readPos - _readLen + (long)_s.Position));
    ...
    private void FlushWrite()
    {
      _awaiter.Await(_s.WriteAsync(_buffer.AsBuffer(0, _writePos)));
      _writePos = 0;
      _awaiter.Await(_s.FlushAsync());
    }
    ...
    public override int Read([In, Out] byte[] array, int offset, int count)
    {
      ...
      if (n == 0)
      {
        if (!_canRead) ErrorReadNotSupported();
        if (_writePos > 0) FlushWrite();
        if (count >= _bufferSize)
        {
          n = (int)_awaiter.Await(_s.ReadAsync(array.AsBuffer(offset, count), (uint)count, InputStreamOptions.Partial)).Length;
          // Throw away read buffer. 
          _readPos = 0;
          _readLen = 0;
          return n;
        ...
        if (_buffer == null) _buffer = new byte[_bufferSize];
        n = (int)_awaiter.Await(_s.ReadAsync(_buffer.AsBuffer(), (uint)_bufferSize, InputStreamOptions.Partial)).Length;
        if (n == 0) return 0;
        _readPos = 0;
        _readLen = n;
      }
      ...
      if (n < count)
      {
        int moreBytesRead = (int)_awaiter.Await(_s.ReadAsync(array.AsBuffer(offset + n, count - n), (uint)(count - n), InputStreamOptions.Partial)).Length;
        n += moreBytesRead;
        _readPos = 0;
        _readLen = 0;
      }
    ...
    {
      if (_s == null) ErrorStreamIsClosed();
      if (_readLen == 0 && !_canRead) ErrorReadNotSupported();
      if (_readPos == _readLen)
      {
        if (_writePos > 0) FlushWrite();
        if (_buffer == null) _buffer = new byte[_bufferSize];
        _readLen = (int)_awaiter.Await(_s.ReadAsync(_buffer.AsBuffer(), (uint)_bufferSize, InputStreamOptions.Partial)).Length;
        _readPos = 0;
      }
      if (_readPos == _readLen) return -1;
    ...
    public override void Write(byte[] array, int offset, int count)
    {
      ...
      if (count >= _bufferSize)
      {
        _awaiter.Await(_s.WriteAsync(array.AsBuffer(offset, count)));
        return;
      }
      else if (count == 0)
        return;  // Don't allocate a buffer then call memcpy for 0 bytes. 
    ...
    public override long Seek(long offset, SeekOrigin origin)
    {
      ...
      return _s.Seek(offset, origin);
      */
      long oldPos = (long)_s.Position + (_readPos - _readLen);
      switch (origin)
      {
        case SeekOrigin.Begin:
          _s.Seek((ulong)offset); break;

        case SeekOrigin.Current:
          _s.Seek((ulong)(offset + (long)_s.Position)); break;

        default:
          _s.Seek((ulong)(offset + (long)_s.Size)); break;
      }
      long pos = (long)_s.Position;
      ...
      if (_readLen > 0)
      {
        ...
        if (oldPos == pos)
        {
          ...
          // position so our Position property is correct.
          if (_readLen > 0)
            _s.Seek((ulong)_readLen + _s.Position);
        }
        else if (oldPos - _readPos < pos && pos < oldPos + _readLen - _readPos)
        {
          int diff = (int)(pos - oldPos);
    ...
    public override void SetLength(long value)
    {
      ...
      if (_s == null) ErrorStreamIsClosed();
      if (!_canSeek) ErrorSeekNotSupported();
      if (!_canWrite) ErrorWriteNotSupported();
      if (_writePos > 0)
      {
        FlushWrite();
      }
